...
| Output key | Purpose |
| --- | --- |
| `ssh_host` | Hostname or IP address reachable from the CI runner. |
...
`chef/scripts/gen_env_from_terraform.sh` reads the outputs above and writes a `.env` file consumed by Test Kitchen. Missing `ssh_host`, `ssh_user`, or `ssh_port` will cause the script to exit with an error.
